...
累加训练至 **2019 年**（2016-2019）→ 在 **2022 年**测试集上，APIgraph (ENS) 达到 **97.10% 准确率**，F1 值为 **0.8800**。
相同的累加训练条件下，传统的 API 直接向量 (Drebin 类似方法) 准确率仅为 85.64%，F1 值为 0.3830。

## 核心贡献

| 贡献点 | 说明 |
| :--- | :--- |
| 提出 **APIgraph** 表示方法 | 基于 API 调用序列构建有向图，固定 2000 维特征，**有效提升跨年检测的鲁棒性**。 |
| 严格的**时序实验设计** | 2016~2019 年累加训练，2020、2021、2022 年测试，严格模拟概念漂移场景。 |
| **效果显著优于Drebin** | 在 2022 年测试集上，相同累加训练条件下，APIgraph (ENS) 相比 Drebin 类似方法，准确率提升 **11.46%** ($97.10\% \text{ vs } 85.64\%$)，F1 值提升超过一倍。 |
| 集成学习增强 | 采用 SVM + Random Forest **软投票集成** (ENS) 提高最终性能。 |
| 完整复现脚本 + 数据对齐逻辑 | 运行脚本可得到不同模型、不同年份的详细对比图表。 |

## 实验结果一览（累加训练至 2019 年 $\rightarrow$ 跨年测试）

本项目主要展示 APIgraph 在面对跨年“概念漂移”问题时的优越性。以下为 **2016-2019 年累加训练**后，在不同年份测试集上的部分结果对比。

### 准确率 (Accuracy) 对比

| 测试年份 | Drebin 类似 (无 Graph) - ENS | **APIgraph (有 Graph) - ENS** |
| :--- | :--- | :--- |
| 2020 | 0.9100 | **0.9450** |
| 2021 | 0.8995 | **0.9950** |
| **2022** | 0.8564 | **0.9710** |
...
> 结论：**APIgraph 在跨年检测中展现出极强的时序鲁棒性。** 在 2022 年测试集上，当 Drebin 类似方法的性能因概念漂移而急剧下降时，**APIgraph 仍能保持 97.10% 的高准确率**，彻底解决了传统稀疏特征随时间快速失效的问题。
